...
export async function markAsSolution(rawCommentId, rawTopicId) {
  ...
  try {
    ...
    const [existingSolutions] = await connection.query(
      'SELECT id FROM comments WHERE topic_id = ? AND is_solution = 1 FOR UPDATE',
      [topicId],
    );
    ...
    if (existingSolutions.length === 0 && selectedComment.user_id !== user.id) {
...
export async function votePoll(rawPollId, rawOptionId) {
  ...
  enforceRateLimit(`poll:${user.id}`, { limit: 30, windowMs: 60 * 1000 });
  const connection = await db.getConnection();
  let firstVote = false;
...
export async function createTopicWithPoll(formData) {
  const user = await requireUser();
  enforceRateLimit(`create-topic:${user.id}`, { limit: 5, windowMs: 10 * 60 * 1000 });
  ...
  let upload = null;
  try {
    upload = await uploadImage(formData.get('image'), 'byteboard_topics');
  } catch {
...
export async function updateTopic(formData) {
  ...
  try {
    const upload = await uploadImage(formData.get('image'), 'byteboard_topics');
...
export async function updateAvatar(formData) {
  ...
  let upload;
  try {
    upload = await uploadImage(formData.get('avatar'), 'byteboard_avatars');
...
export async function chatWithBot(rawHistory, rawMessage) {
  ...
  enforceRateLimit(`ai-chat:${ip}`, { limit: 15, windowMs: 10 * 60 * 1000 });
