...
import { destroyMediaAsset } from './mediaCleanup';
import { deliverFollowTopicNotifications, isNotificationEnabled } from './discovery';
...
export async function markNotificationsAsRead() {
  try {
    ...
    return { success: true, message: 'ทำเครื่องหมายว่าอ่านทั้งหมดแล้ว' };
  } catch (error) {
    console.error('Mark all notifications error:', error);
    return { success: false, message: 'อัปเดตการแจ้งเตือนไม่สำเร็จ กรุณาลองใหม่' };
  }
}

export async function markNotificationAsRead(rawNotificationId) {
  try {
    const user = await requireUser();
    const notificationId = positiveInteger(rawNotificationId, 'notification id');
    await db.query(
...
export async function deleteNotification(rawNotificationId) {
  try {
    const user = await requireUser();
    const notificationId = positiveInteger(rawNotificationId, 'notification id');
    await db.query('DELETE FROM notifications WHERE id = ? AND user_id = ?', [notificationId, user.id]);
    revalidatePath('/notifications');
    revalidatePath('/', 'layout');
    return { success: true, message: 'ลบการแจ้งเตือนแล้ว' };
  } catch (error) {
    console.error('Delete notification error:', error);
    return { success: false, message: 'ลบการแจ้งเตือนไม่สำเร็จ กรุณาลองใหม่' };
  }
}
...
export async function deleteAllNotifications() {
  try {
    const user = await requireUser();
    await db.query('DELETE FROM notifications WHERE user_id = ?', [user.id]);
    revalidatePath('/notifications');
    revalidatePath('/', 'layout');
    return { success: true, message: 'ล้างการแจ้งเตือนทั้งหมดแล้ว' };
  } catch (error) {
    console.error('Delete all notifications error:', error);
    return { success: false, message: 'ล้างการแจ้งเตือนไม่สำเร็จ กรุณาลองใหม่' };
  }
}
...
export async function markAsSolution(rawCommentId, rawTopicId) {
  ...
  const notificationMessage = `${user.username} เลือกคำตอบของคุณเป็นคำตอบที่ช่วยแก้ปัญหา`;
  ...
  try {
    ...
    if (selectedComment.user_id !== user.id) {
      ...
      if (selectedComment.user_id
        && await isNotificationEnabled(selectedComment.user_id, 'solutions_enabled', connection)) {
        const [notificationResult] = await connection.query(
          'INSERT INTO notifications (user_id, actor_id, topic_id, type, message) VALUES (?, ?, ?, ?, ?)',
          [selectedComment.user_id, user.id, topicId, 'solution', notificationMessage],
        );
        notificationId = notificationResult.insertId;
        notificationRecipientId = selectedComment.user_id;
      }
    }
    await connection.commit();
  } catch (error) {
    ...
    return { success: false, message: error?.message === 'Forbidden' ? 'คุณไม่มีสิทธิ์เลือกคำตอบนี้' : 'บันทึกคำตอบไม่สำเร็จ กรุณาลองใหม่' };
  } finally {
    connection.release();
  }
  ...
  if (notificationId && notificationRecipientId) {
    try {
      await pusherServer.trigger(notificationChannelName(notificationRecipientId), 'new-notification', {
        id: notificationId,
        topic_id: topicId,
        type: 'solution',
      ...
      console.error('Solution notification delivery failed');
    }
  }
...
export async function votePoll(rawPollId, rawOptionId) {
  ...
  const [updatedOptions] = await db.query(
...
export async function createTopicWithPoll(formData) {
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO topics (title, category, content, user_id, image_url, image_public_id)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [input.title, input.category, input.content, user.id, upload?.secure_url || null, upload?.public_id || null],
    );
    ...
    await connection.rollback();
    if (upload?.public_id) await destroyMediaAsset(upload.public_id, 'topic_create_failed');
    console.error('Create Topic Error:', error);
    return { success: false, message: 'สร้างกระทู้ไม่สำเร็จ กรุณาลองใหม่' };
  } finally {
    connection.release();
  }

  after(() => generateAiReply(newTopicId, input.title, plainText(input.content)).catch(console.error));
  after(() => deliverFollowTopicNotifications({
    topicId: newTopicId,
    actorId: user.id,
    actorUsername: user.username,
    category: input.category,
  }).catch(() => console.error('Follow notification fan-out failed')));
...
export async function updateTopic(formData) {
  ...
  const [topics] = await db.query(
    'SELECT user_id, image_url, image_public_id FROM topics WHERE id = ?',
    [topicId],
  );
  ...
  let newImagePublicId = formData.get('isImageRemoved') === 'true' ? null : topic.image_public_id;
  let upload = null;
  try {
    upload = await uploadImage(formData.get('image'), 'ithub_topics');
    if (upload) {
  ...
  try {
    await db.query(
      `UPDATE topics
       SET title = ?, category = ?, content = ?, image_url = ?, image_public_id = ?
       WHERE id = ?`,
      [input.title, input.category, input.content, newImageUrl, newImagePublicId, topicId],
    );
  } catch (error) {
    if (upload?.public_id) await destroyMediaAsset(upload.public_id, 'topic_update_failed');
    console.error('Update Topic Error:', error);
    return { success: false, message: 'บันทึกการแก้ไขไม่สำเร็จ กรุณาลองใหม่' };
  }
  if (topic.image_public_id && topic.image_public_id !== newImagePublicId) {
    after(() => destroyMediaAsset(topic.image_public_id, 'topic_image_replaced'));
  }
  revalidatePath(`/topic/${topicId}`);
...
export async function updateAvatar(formData) {
  const user = await requireUser();
  const [users] = await db.query('SELECT avatar_public_id FROM users WHERE id = ?', [user.id]);
  ...
  try {
    await db.query(
      ...
      [upload.secure_url, upload.public_id, user.id],
    );
  } catch (error) {
    await destroyMediaAsset(upload.public_id, 'avatar_update_failed');
    console.error('Update avatar error:', error);
    return { success: false, message: 'อัปเดตรูปโปรไฟล์ไม่สำเร็จ กรุณาลองใหม่' };
  }
  if (previousPublicId && previousPublicId !== upload.public_id) {
    after(() => destroyMediaAsset(previousPublicId, 'avatar_replaced'));
  }
