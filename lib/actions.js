...
// --- 7. ฟังก์ชันสร้างกระทู้ (พร้อมโพล & 🚀 อัปโหลดรูป Cloudinary) ---
export async function createTopicWithPoll(formData) {
  ...
  if (imageFile && imageFile.size > 0) {
      // 🚀 แปลงไฟล์เป็น Base64 แล้วส่งขึ้น Cloudinary (แบบปลอดภัย)
      const buffer = Buffer.from(await imageFile.arrayBuffer());
      // ตรวจสอบ type ของไฟล์ให้ชัวร์
      const mimeType = imageFile.type || 'image/jpeg';
      const base64Image = `data:${mimeType};base64,${buffer.toString('base64')}`;
      
      try {
        // ใส่ await และจับผลลัพธ์แบบเป๊ะๆ
        const uploadResponse = await cloudinary.uploader.upload(base64Image, {
          folder: 'byteboard_topics', 
        });
        
        // ชี้เป้า URL ให้ชัดเจน
...
// --- 8. ✅ ฟังก์ชันแก้ไขกระทู้ (Update Topic & 🚀 อัปโหลดรูป Cloudinary) ---
  ...
  try {
    ...
    if (imageFile && imageFile.size > 0) {
        ...
        try {
          ...
          newImageUrl = uploadResponse.secure_url || uploadResponse.url;
        } catch (uploadError) {
          console.error("Cloudinary Upload Error:", uploadError);
        }
    }
...
// --- ฟังก์ชันคุยแชทส่วนตัวกับ Bot (อัปเกรดความจำ) ---
export async function chatWithBot(history, message) {
  'use server';
  
  const { askGeminiChat } = require('./gemini'); 

  try {
    const reply = await askGeminiChat(history, message);
    return reply;
  } catch (error) {
