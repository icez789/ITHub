...
import { externalServiceEnabled } from './externalServicesCore';

const fallbackReply = 'ขออภัยครับ ระบบ AI กำลังพักผ่อนอยู่ 🤖💤';
const geminiEnabled = externalServiceEnabled(process.env.GEMINI_API_KEY);
const genAI = geminiEnabled ? new GoogleGenerativeAI(process.env.GEMINI_API_KEY) : null;
...
export async function askGeminiChat(history, newMessage) {
  if (!genAI) return fallbackReply;
  // สร้างตัวแปรดึงเวลาปัจจุบัน (กำหนดโซนเวลาไทยเพื่อความแม่นยำ)
  const currentDateTime = new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', dateStyle: 'full', timeStyle: 'medium' });

  const systemInstruction = `คุณคือ "ITHub Bot 🤖" ผู้ช่วยอัจฉริยะประจำเว็บบอร์ด ITHub
  ...
  ข้อมูลสำคัญ: ขณะนี้คือ ${currentDateTime} (โปรดอ้างอิงข้อมูลให้เป็นปัจจุบันเสมอ)`;
