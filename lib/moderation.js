...
export async function deleteTopicCascade(topicId, audit = null) {
  const connection = await db.getConnection();
  try {
    await connection.beginTransaction();
    const [topics] = await connection.query('SELECT user_id, image_public_id FROM topics WHERE id = ? FOR UPDATE', [topicId]);
    if (!topics[0]) {
    ...
    if (audit?.actorId) {
      await writeModerationAudit({
        ...
        action: audit.action || 'topic.delete',
        targetType: 'topic',
        targetId: topicId,
        metadata: { ownerId: topicOwnerId },
      });
    }
    await connection.commit();
    return { deleted: result.affectedRows > 0, imagePublicId: topics[0].image_public_id || null };
  } catch (error) {
    await connection.rollback();
    throw error;
  } finally {
...
export async function deleteCommentCascade(commentId, audit = null) {
  const connection = await db.getConnection();
  try {
    await connection.beginTransaction();
    ...
    if (audit?.actorId) {
      await writeModerationAudit({
        executor: connection,
        actorId: audit.actorId,
        action: audit.action || 'comment.delete',
        targetType: 'comment',
        targetId: commentId,
        metadata: { authorId: comment.user_id },
      });
    }
    await connection.commit();
...
export async function setTopicModerationState(topicId, type, enabled, actorId) {
  if (!['pin', 'lock'].includes(type)) throw new Error('Invalid moderation action');
  const connection = await db.getConnection();
  try {
    await connection.beginTransaction();
    const [topics] = await connection.query(
      'SELECT is_pinned, is_locked FROM topics WHERE id = ? FOR UPDATE',
      [topicId],
    );
    if (!topics[0]) {
      await connection.rollback();
    ...
    const prefix = type === 'pin' ? 'pinned' : 'locked';
    await connection.query(
      `UPDATE topics
       SET is_${prefix} = ?, ${prefix}_by = ?, ${prefix}_at = ?
       WHERE id = ?`,
      [enabled ? 1 : 0, enabled ? actorId : null, enabled ? new Date() : null, topicId],
    );
    await writeModerationAudit({
      executor: connection,
      actorId,
      action: `topic.${type}.${enabled ? 'enable' : 'disable'}`,
      targetType: 'topic',
      targetId: topicId,
    });
    await connection.commit();
