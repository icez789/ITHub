import Link from 'next/link';
import Image from 'next/image';
import { Eye, Heart, Lock, MessageCircle, Pin, Sparkles, UserRound } from 'lucide-react';
import ResearchTrackedLink from './ResearchTrackedLink';

export default function TopicCard({
  id,
  title,
  category,
  excerpt,
  username,
  createdAt,
  imageUrl,
  views = 0,
  commentCount = 0,
  likeCount = 0,
  index = 0,
  isPinned = false,
  isLocked = false,
  recommendationReason = null,
  searchResultPosition = null,
}) {
  const formattedDate = new Date(createdAt).toLocaleDateString('th-TH', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  });
  const linkClass = 'ithub-card flex min-w-0 gap-3 p-3.5 transition-[border-color,transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-[var(--app-primary)] hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-focus-ring)] sm:gap-4 sm:p-4';
  const cardContent = <>
    <div className="min-w-0 flex-1">
      <div className="mb-1.5 flex flex-wrap items-center gap-2">
        <span className="rounded-lg bg-[var(--app-primary-soft)] px-2 py-0.5 text-[11px] font-bold text-[var(--app-accent-text)]">
          {category}
        </span>
        {isPinned ? <span className="inline-flex items-center gap-1 rounded-lg bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-800 dark:bg-amber-950/35 dark:text-amber-300"><Pin aria-hidden="true" size={11} /> ปักหมุด</span> : null}
        {isLocked ? <span className="inline-flex items-center gap-1 rounded-lg bg-zinc-100 px-2 py-0.5 text-[11px] font-bold text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"><Lock aria-hidden="true" size={11} /> ล็อกแล้ว</span> : null}
        {recommendationReason ? <span className="inline-flex items-center gap-1 rounded-lg border border-[var(--app-border)] bg-[var(--app-surface-subtle)] px-2 py-0.5 text-[11px] font-semibold text-[var(--app-text-muted)]"><Sparkles aria-hidden="true" size={11} /> {recommendationReason}</span> : null}
        <span className="text-xs text-[var(--app-text-muted)]">{formattedDate}</span>
      </div>

      <h3 className="line-clamp-2 text-base font-bold leading-snug text-[var(--app-text)] transition-colors group-hover:text-[var(--app-accent-text)] sm:text-lg">
        <span data-tour="topic-card-focus">{title}</span>
      </h3>

      {excerpt && (
        <p className="mt-1.5 hidden line-clamp-2 text-sm leading-6 text-[var(--app-text-muted)] sm:block">
          {excerpt}
        </p>
      )}

      <div className="mt-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-2 text-xs text-[var(--app-text-muted)]">
        <span className="flex min-w-0 items-center gap-1.5">
          <UserRound aria-hidden="true" className="h-3.5 w-3.5 shrink-0" />
          <span className="max-w-36 truncate">{username || 'ไม่ระบุผู้เขียน'}</span>
        </span>
        <span className="flex items-center gap-3" aria-label={`${views} ครั้ง, ${commentCount} ความคิดเห็น, ${likeCount} ถูกใจ`}>
          <span className="flex items-center gap-1"><Eye aria-hidden="true" className="h-3.5 w-3.5" />{Number(views).toLocaleString('th-TH')}</span>
          <span className="flex items-center gap-1"><MessageCircle aria-hidden="true" className="h-3.5 w-3.5" />{Number(commentCount).toLocaleString('th-TH')}</span>
          <span className="flex items-center gap-1"><Heart aria-hidden="true" className="h-3.5 w-3.5" />{Number(likeCount).toLocaleString('th-TH')}</span>
        </span>
      </div>
    </div>

    {imageUrl && (
      <div className="relative h-[66px] w-[88px] shrink-0 overflow-hidden rounded-xl bg-[var(--app-surface-subtle)] sm:h-24 sm:w-36">
        <Image
          src={imageUrl}
          alt=""
          fill
          sizes="(max-width: 639px) 88px, 144px"
          preload={index === 0}
          className="object-cover transition-transform duration-200 group-hover:scale-[1.02]"
        />
      </div>
    )}
  </>;

  return (
    <article className="group" data-tour="topic-card">
      {searchResultPosition
        ? <ResearchTrackedLink
            href={`/topic/${id}`}
            analyticsEvent={{
              eventName: 'search_result_opened',
              properties: { resultPosition: searchResultPosition },
            }}
            data-tour="topic-link"
            className={linkClass}
          >
            {cardContent}
          </ResearchTrackedLink>
        : <Link href={`/topic/${id}`} data-tour="topic-link" className={linkClass}>{cardContent}</Link>}
    </article>
  );
}
